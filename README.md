...
| **清單決策面板** | **清單頁每檔顯示便宜度色條（0% 便宜 → 100% 昂貴）+ 當日漲跌，可一鍵「依便宜度排序」一眼掃出全市場誰最便宜** |
| **到價提醒** | **個股可設目標價，跌破時下次開 app 在清單頁置頂提醒（純前端，存 localStorage、無需後端）** |
...
| PWA | 可安裝到主畫面、離線可用、**自動更新**（新版部署後 1 次 reload 內套用，會跳「新版本已就緒」橫條讓你按下重新載入；不必手動清快取） |
